...
def plot_price_with_regime(log_path="logs/signal_log.jsonl"):
    try:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=merged["timestamp"], y=merged["Close"], mode="lines", name="Price", line=dict(color="black")))
        ...
        for i in range(len(merged)):
            ...
            if curr["regime"] != last_regime:
                if last_regime and start_time:
                    fig.add_vrect(x0=start_time, x1=curr["timestamp"], fillcolor=colors.get(last_regime, "gray"), opacity=0.2, line_width=0)
                start_time = curr["timestamp"]
                last_regime = curr["regime"]
        if start_time and last_regime:
            fig.add_vrect(x0=start_time, x1=merged["timestamp"].iloc[-1], fillcolor=colors.get(last_regime, "gray"), opacity=0.2, line_width=0)

        buys = merged[merged["signal"] == "Buy"]
        sells = merged[merged["signal"] == "Sell"]
        fig.add_trace(go.Scatter(x=buys["timestamp"], y=buys["Close"], mode="markers", name="Buy Signal",
                                 marker=dict(symbol="triangle-up", color="green", size=10)))
        fig.add_trace(go.Scatter(x=sells["timestamp"], y=sells["Close"], mode="markers", name="Sell Signal",
                                 marker=dict(symbol="triangle-down", color="red", size=10)))

        fig.update_layout(title="📊 Price with Regimes & Signals", xaxis_title="Date", yaxis_title="Price", height=400)
...
def simulate_strategy_vs_hold(log_path="logs/signal_log.jsonl"):
    try:
        ...
        merged["signal"].ffill(inplace=True)
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=merged["timestamp"], y=merged["equity_curve"], mode="lines", name="📈 Strategy"))
        fig.add_trace(go.Scatter(x=merged["timestamp"], y=merged["buy_hold_curve"], mode="lines", name="📊 Buy & Hold", line=dict(dash="dot")))
        fig.update_layout(title="📈 Strategy vs Buy & Hold Performance", xaxis_title="Date", yaxis_title="Cumulative Return", height=400)
        st.plotly_chart(fig, use_container_width=True)
